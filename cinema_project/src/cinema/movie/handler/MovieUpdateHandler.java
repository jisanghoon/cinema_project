...
public class MovieUpdateHandler implements CommandHandler {
	...
	public String process(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		if (req.getMethod().equalsIgnoreCase("get")) {
			try (Connection conn = ConnectionProvider.getConnection()) {
				...
				String[] gerneList = movie.getGenre().split(",");

				req.setAttribute("gerneList", gerneList);
			...
			try (Connection conn = ConnectionProvider.getConnection()) {
				...
				int movieNo = Integer.parseInt(multi.getParameter("movieNo"));
