...
public class BookingDao {
	...
	public List<BookTotal> listAll(Connection conn) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				...
				/*
				 * do { Theater theater = new Theater(rs.getInt("theater_no"),
				 * rs.getString("theater_name")); Auditorium auditorium = new
				 * Auditorium(rs.getString("audi_name"),
				 * rs.getString("audi_type"), rs.getString("floor"), theater);
				 * Movie movie = new Movie(rs.getInt("movie_no"),
				 * rs.getString("title_kor"), rs.getString("title_eng"),
				 * rs.getString("director"), rs.getString("actors"),
				 * rs.getDate("release_date"), rs.getString("age_require"),
				 * rs.getInt("time_length"), rs.getString("country"),
				 * rs.getInt("ratings"), rs.getInt("total_attendance"),
				 * rs.getString("genre"), rs.getString("content"),
				 * rs.getString("small_pic_url"), rs.getString("big_pic_url"));
				 * Screen screen = new Screen(rs.getInt("screen_no"), movie,
				 * rs.getString("screen_mode"), rs.getDate("buy_date"),
				 * rs.getLong("screen_price"), rs.getString("supplier"),
				 * rs.getDate("start_date"), rs.getDate("end_date")); Schedule
				 * schedule = new Schedule(rs.getInt("schedule_no"),
				 * rs.getTime("start_time").toLocalTime(),
				 * rs.getTime("end_time").toLocalTime(),
				 * rs.getDate("show_date"), auditorium, screen); Seat seat = new
				 * Seat(rs.getInt("seat_no"), rs.getInt("row"),
				 * rs.getInt("col"), rs.getString("seat_name"), auditorium);
				 * 
				 * bookTotal = new BookTotal(schedule, seat, auditorium,
				 * theater, screen, movie); bookTotals.add(bookTotal); } while
				 * (rs.next());
				 */
				return bookTotals;
			} else {
	...
	public List<Seat> selSeat(Connection conn, int audiNo) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				...
				/*
				 * do { Auditorium auditorium = new
				 * Auditorium(rs.getInt("audi_no")); seat = new
				 * Seat(rs.getInt("seat_no"), rs.getInt("row"),
				 * rs.getInt("col"), rs.getString("seat_name"), auditorium);
				 * seats.add(seat); } while (rs.next());
				 */
				return seats;
			} else {
				return Collections.emptyList();
	...
	public Booking insertBook(Connection conn, int cusNo, int priceNo, int scheduleNo) {
		return null;
